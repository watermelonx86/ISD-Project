...
export function EditEmployee(
    {isOpen, handleClose, handleSubmit, editRef,
     id, fname, setFName, lname, setLName, email, setEmail, position, setPosition, username, setUsername, password, setPassword}) 
{
    return (
        <div className={`${isOpen ? "block" : "hidden"}`}>
            <div className= "fixed top-0 left-0 w-full h-full bg-overLay opacity-50 z-[200]"></div>
            <div className="fixed flex left-0 right-0 z-[250] items-center justify-center overflow-x-hidden overflow-y-auto top-4 
                    md:inset-0 h-modal sm:h-full">
                <div ref={editRef} className="relative w-full h-full max-w-2xl px-4 md:h-auto">
                    {/* <!-- Modal content --> */}
                            ...
                            <form action="#" onSubmit={(e) => handleSubmit(e)}>
                                <div className="grid grid-cols-6 gap-6 mb-7">
                                    ...
                                    <div className="col-span-6 sm:col-span-3">
                                        ...
                                        <input type="text" name="position" id="position" 
                                            className="shadow-sm bg-gray-200 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" 
                                            placeholder="e.g. MaNV / ID" 
                                            value={username}
                                            onChange={(e) => setUsername(e.target.value)}
                                            disabled />
                                    </div>
                                    <div className="col-span-6 sm:col-span-3">
                                        <label htmlFor="position" className="block mb-2 text-sm font-medium text-gray-900">New Password</label>
                                        <input type="text" name="position" id="position" 
                                            className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" 
